...
task UpdateHelp {
    Import-Module .\code\PoShSymcDLP.psd1 -Force
    #New-MarkdownHelp -Module PoShSymcDLP -OutputFolder $PSScriptRoot\docs #-Force
...
task CopyModuleFiles {
    ...
    if (-not (Test-Path .\output\PoShSymcDLP)) {
        $null = New-Item -Path .\output\PoShSymcDLP -ItemType Directory
    }

    Copy-Item -Path '.\code\en-US\' -Filter *.* -Recurse -Destination .\output\PoShSymcDLP -Force
    #Copy-Item -Path '.\lib\' -Filter *.* -Recurse -Destination .\output\PoShSymcDLP -Force
    Copy-Item -Path '.\code\public\' -Filter *.* -Recurse -Destination .\output\PoShSymcDLP -Force
    #Copy-Item -Path '.\tests\' -Filter *.* -Recurse -Destination .\output\PoShSymcDLP -Force
    
    #Copy Module Manifest files
    Copy-Item -Path @(
        '.\code\README.md'
        '.\code\PoShSymcDLP.psd1'
        '.\code\PoShSymcDLP.psm1'
    ) -Destination .\output\PoShSymcDLP -Force        
}
#endregion
...
task UpdateManifest {
    ...
    $manifestPath = '.\code\PoShSymcDLP.psd1'
    ...
    Update-ModuleManifest -ModuleVersion $newVersion -Path .\code\PoShSymcDLP.psd1 -ReleaseNotes $ReleaseNotes
}
#endregion
...
task PublishModule -If ($Configuration -eq 'Production') {
    Try {
        ...
        $params = @{
            Path        = ('{0}\Output\PoShSymcDLP' -f $PSScriptRoot )
            NuGetApiKey = $env:psgallery
            ErrorAction = 'Stop'
        }
        Publish-Module @params
        Write-Output -InputObject ('PoShSymcDLP PowerShell Module version published to the PowerShell Gallery')
    }
    Catch {
        throw $_
    }
...
task UpdateVersion {
    ...
    if ($null -ne $publishedModule)
    {
        ...
        Update-ModuleManifest -Path .\code\PoShSymcDLP.psd1 -ModuleVersion $NewVersion
    }
}
#endregion
